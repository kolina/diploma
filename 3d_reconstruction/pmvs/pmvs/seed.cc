...
void Cseed::collectCells(const int index0, const int index1,
  ...
  Mat3 F;
  PMVSImage::setF(m_fm.m_pss.m_photos[index0], m_fm.m_pss.m_photos[index1],
              F, m_fm.m_level);
...
void Cseed::collectCandidates(const int index, const std::vector<int>& indexes,
  ...
  for (int i = 0; i < (int)indexes.size(); ++i) {        
    ...
    Mat3 F;
    PMVSImage::setF(m_fm.m_pss.m_photos[index], m_fm.m_pss.m_photos[indexid],
                F, m_fm.m_level);
	...
	continue;
      ...
      while (begin != end) {
        ...
        if (m_fm.m_epThreshold <= PMVSImage::computeEPD(F, p0, p1)) {
          ++begin;          
          continue;
        }
        vcp.push_back(*begin);
